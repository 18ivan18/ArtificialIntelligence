...
import {calculateWinner, findBestMove} from "./minimax_alpha_beta_pruning/minimax"
...
// }
  ...
  const handleClick = (i, j) => {
    ...
    // obv works
    const squares = Array(3).fill().map(() => Array(3).fill(null));
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
  ...
  useEffect(() => {
    if(!state.xIsNext && !state.result && state.history.length - 1 === state.stepNumber) {
      // const computerMove = generateComputerMove(state.history[state.stepNumber].squares);
      const computerMove = findBestMove(state.history[state.stepNumber].squares, false);
  ...
  return (
    <div className="App">
      <Board board={getCurrent()} reset={() => reset()} updateBoard={(i, j) => handleClick(i, j)} 
              winningSquares={state.result && state.result.status === "win" ? state.result.win.squares : []}></Board>
      <Statistics state={state} setState={setState} jumpTo={jumpTo} reset={() => reset()}></Statistics>
    </div>
  );
}
